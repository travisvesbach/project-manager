<template>
    <button class="w-full relative flex flex-col focus:outline-none focus:border-0 my-2 p-2 rounded-lg" :class="this.task.completed_at ? 'text-secondary-color card-secondary-color' : 'text-color card-color drag-task'" title="details" @click="$emit('show')">
        <div>
            <p class="p-0 text-lg text-left">
                <svg class="h-5 -mt-1 hover:text-green-500 inline-block" :class="task.completed_at ? 'text-green-500' : ''" @click="toggleCompleted()" v-on:click.stop xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
